...
if __name__ == '__main__':
    ...
    f1 = open('exp_real_'+dset_name+'/records.txt','a')
    ...
    #with open('exp_real_'+dset_name+'/beta_%s_model_%s_epoch_%s_norm_%s_fix_seed_%s_r_%s_p_%s.txt' 
    ...
    if dset_name == 'fgnet':
        ...
        f1.write("beta: %s, model: %s, simp_loss: %s, epoch: %s, use_norm: %s, fix_data_seed: %s, fix_train_seed: %s, avg: %s %s %s, acc_std: %s %s %s, cv_fold: %s\n"% (str(beta), str(model_name), str(simp_loss), str(num_epoch), str(use_norm), str(fix_data_seed), str(fix_train_seed), str(acc_avg), str(mae3_avg), str(mae5_avg), str(acc_std), str(mae3_std), str(mae5_std), str(cv_fold)))
        f1.flush()
        f1.close()
    else:
        acc_avg = np.mean(accs)
        acc_std = np.std(accs)
        f1.write("beta: %s, model: %s, simp_loss: %s, epoch: %s, use_norm: %s, fix_data_seed: %s, fix_train_seed: %s, acc_avg: %s, acc_std: %s, cv_fold: %s\n"% (str(beta), str(model_name), str(simp_loss), str(num_epoch), str(use_norm), str(fix_data_seed), str(fix_train_seed), str(acc_avg), str(acc_std), str(cv_fold)))
        f1.flush()
        f1.close()
