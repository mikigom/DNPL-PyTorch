...
parser.add_argument('-fsp_iden', required=False)
...
fsp_iden = True if args.fsp_iden == None else args.fsp_iden.lower() in ('true', '1', 't', 'y')
...
if __name__ == '__main__':
    ...
    acc_std = np.std(accs)
    f.write("runid: %s, p: %s, beta: %s beta2: %s, model: %s, fsp_iden: %s, epoch: %s, use_norm: %s, fix_data_seed: %s, fix_train_seed: %s, acc_avg: %s, acc_std: %s, cv_fold: %s\n" % (runid, str(p), str(beta), str(beta2), str(model_name), str(fsp_iden), str(num_epoch), str(use_norm), str(fix_data_seed), str(fix_train_seed), str(acc_avg), str(acc_std), str(cv_fold)))
    f.flush()
    f.close()
